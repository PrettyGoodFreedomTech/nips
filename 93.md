...
The `z` tag is required by all kind `9999` and `39999` events. It is used to indicate the event id of the parent list (the list header, a kind `(3)9998` event). 

It `should` have at least one item tag, which is a `p`, `e`, `t`, or `a` tag (others), as indicated by `required` by the parent list declaration.
