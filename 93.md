...
We introduce a single event kind: `9999`, which is used either to declare a new list or to add an item to a list.

## Tags

The `z` tag is required and defines the category or categories to which the kind `9999` note belongs. Typically the `z` tag is `list` when declaring a new list, or is the list event id when declaring an item on a list. In general, `z` must be a string, and can be either human readable or an event id. If an item belongs to multiple lists, there will be one `z` tag for each list to which it should belong.

The `p`, `e`, `t`, and `a` tags are used when declaring a pubkey, event id, string, or naddr to a list. The parent list declaration event `should` specify which of these tags are expected to be present in their children using the `required`, `allowed` and `disallowed` tags.
...
The composite list can be composed 
